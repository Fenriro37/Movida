...
class AVL {
	...
	/* trova il Nodo con chiave massima nel sottoalbero destro
	 * usata anche in ModivaCore per la ricerca searchMostVotedMovies
	 */
	...
	private void rotazioneSinistra(Nodo x) {
		Nodo y = x.right;
		x.right = y.left;
		if(y.left != null) {
			y.left.parent = x;
	...
	private void rotazioneDestra(Nodo x) {
		Nodo y = x.left;
		x.left = y.right;
		if(y.right != null) {
			y.right.parent = x;
	...
	private int sbilanciamento(Nodo n) {
		if(n == null)
			return 0;
	...
	private void sostituisci(Nodo u, Nodo v) { //u rimpiazzato da v, sostituzione
	...
	private void aggiustaDelete(Nodo n) {		
		while(n != null) {
			n.height = 1 + max(height(n.left), height(n.right));
			
			if(sbilanciamento(n) <= -2 || sbilanciamento(n) >= 2) { //il nonno è sbilanciato
				Nodo x, y, z;
				x = n;
	...
	/*	Funzioni supplementari ________________________________________________________
	 * 	
	...
	/* Usata dalla funzione: Delete in MovidaCore
	...
	private void Stampa_util (Nodo v) {
	    if (v != null) {
